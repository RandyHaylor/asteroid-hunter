...
describe('applyAvoidancePushback', () => {
  it('pushes the ship outward (away from the asteroid), even head-on', () => {
    ...
    for (let step = 0; step < 10; step++) {
      applyAvoidancePushback(player, asteroidPosition, 1, 1 / 60)
    }
    expect(player.distanceTo(asteroidPosition)).toBeGreaterThan(distanceBefore) // shoved outward
    expect(player.x).toBeGreaterThan(60)
  })

  it('does nothing at zero proximity', () => {
    const player = new Vector3(60, 0, 0)
    applyAvoidancePushback(player, new Vector3(0, 0, 0), 0, 1 / 60)
