...
| D75 | **Autopilot settings overlay (phase 2 of 3).** `autopilot/shipAutopilotSettingsPanel.ts` (+ css): a semi-transparent **shaded panel over the radar** (radar visible behind) exposing the live autopilot knobs — approach angle, engage range, isolation (single-out) weight, flee-if->N-in-range, evade-below-shield %, re-engage shield %, target priority (select), flee-after-any-damage (checkbox) — each input mutates `shipAutopilotSettings` live. A **"⌄" caret** (top-right of the controls) shows/hides it; shown by default on entering AI mode, fully hidden in manual. Verified: panel+caret appear only in AI mode, caret collapses, no console errors. (Deferred: the landscape "all controls on the right" refactor — the overlay sits over the radar, which is right-side in both orientations, so it already works consistently without it. Phase 3 (forced wave-3 AI) still pending.) |

| D76 | **Forced-AI wave (phase 3 of 3) — completes AI mode.** Wave **3** is a forced AI-only level: when it goes active, the autopilot is forced on (`setAutopilotModeActive(true)`), the AI toggle is **locked** (`autopilotIsForcedThisWave` blocks it), a full-screen **red flash** plays, and a bold overlay shows the verbatim message: *"WARNING - MALFUNCTION IN MANUAL FLIGHT CONTROLS DETECTED - MANUAL MODE OFFLINE - 'Sorry, captain, looks like you'll have to rely on the ships ai for this wave...'"* (auto-fades after ~5.5 s; the lock persists the whole wave). Manual flight is restored on the next wave. Verified: overlay elements + verbatim text present, hidden by default, no console errors (not seen firing in a live wave-3 run — needs play-test). |
...
| D80 | **Autopilot fixes — use player controls; actually fight.** (1) **Player-control routing (architecture fix)**: the AI no longer rotates the heading with a bespoke direct-rotate (deleted) — it now derives **pitch/yaw inputs** (like the keyboard turn keys) and feeds the SAME `applyPitchYawToCommandedHeading` the player uses, plus the thrust flag and the orbit-latch controller. The AI can do nothing the player can't (bounded to maxTurnRate, two axes). (2) **Engage actually fires**: it now aims **straight at the enemy** when within firing range (so the enemy enters the nose-cone lock and auto-fire triggers) and coasts to strafe; it only arcs from the preferred approach angle while **closing** beyond the preferred range. (Previously it flew to a flank stand-off point and never pointed at the enemy, so it never fired.) (3) **Evade stays in the field**: it heads to and **orbits the nearest asteroid** to shake pursuers instead of fleeing to the boundary and relying on the forced far-orbit. [unit-tested] |
